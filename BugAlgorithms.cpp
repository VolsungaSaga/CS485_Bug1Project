...
Move BugAlgorithms::Bug1(Sensor sensor)
{
    ...
    if ((timesRep > 0) || (timesRep < 0)) {
        ...
        move = getStepVector(xMove, yMove);
         ...
         if ((xGoalPos - m_best[0]) < (xGoalPos - (sensor.m_xmin-xCurr2)))
             m_best[0] = sensor.m_xmin-xCurr2;
         if ((yGoalPos - m_best[1]) < (yGoalPos - (sensor.m_ymin-yCurr2)))
             m_best[1] = sensor.m_ymin-yCurr2;
         else if ((m_best[0] != 1000) && (m_simulator->ArePointsNear(xMove,yMove,m_leave[0],m_leave[1])))
             timesRep += 1;
         else if ((timesRep > 0) && (m_simulator->ArePointsNear(xMove,yMove,m_best[0],m_best[1])))
             timesRep += 1;
